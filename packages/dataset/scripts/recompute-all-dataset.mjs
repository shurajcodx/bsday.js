...
for (let year = 1990; year <= 2100; year++) {
  ...
  const standaloneRules = festivalRules.filter(r => {
    const name = r.name;
    return ![
      'Ghatasthapana', 'Fulpati', 'Maha Ashtami', 'Maha Navami', 'Vijaya Dashami', 'Papankusha Ekadashi', 'Kojagrat Purnima',
      'Kag Tihar', 'Kukur Tihar', 'Laxmi Puja', 'Govardhan Puja / Mha Puja / Gai Puja', 'Bhai Tika', 'Tihar Holiday', 'Chhath Puja', 'Haribodhini Ekadashi / Tulsi Bibaha'
    ].includes(name);
  });
  ...
  for (let i = 0; i < dailyPanchangs.length; i++) {
    ...
    for (const rule of standaloneRules) {
      ...
      if (type === 'sunrise') {
        ...
        if (matchesWindow || matchesSunrise) {
          ...
          let alreadyMatched = false;
          if (prevDay) {
            const prevTargetPanchang = (type === 'sunset') ? prevDay.sunsetPanchang : prevDay.nightPanchang;
            if ((prevTargetPanchang.tithi === cond.tithi && (!cond.paksha || cond.paksha === prevTargetPanchang.paksha)) ||
    ...
    if (day.month === 6 && day.paksha === 'Krishna') {
      if (day.tithi === 'Ashtami' || (dailyPanchangs[i - 1]?.nightPanchang.tithi === 'Ashtami' && day.tithi === 'Navami')) {
    ...
    if (day.month === 5 && day.paksha === 'Shukla') {
      if (day.tithi === 'Tritiya' || (dailyPanchangs[i - 1]?.tithi === 'Tritiya' && day.tithi === 'Chaturthi' && day.day === 29)) {
  ...
  let ghataIdx = dailyPanchangs.findIndex(d => (d.month === 6 || d.month === 7) && d.paksha === 'Shukla' && d.tithi === 'Pratipada');
  if (ghataIdx !== -1) {
    newData[dailyPanchangs[ghataIdx].date].festivals.push('Ghatasthapana');

    let fulpatiIdx = -1, mahaAshtamiIdx = -1, mahaNavamiIdx = -1, vijayaDashamiIdx = -1, ekadashiIdx = -1, purnimaIdx = -1;
    ...
    for (let i = ghataIdx; i < Math.min(ghataIdx + 20, dailyPanchangs.length); i++) {
      ...
      if (p.tithi === 'Ashtami' && mahaAshtamiIdx === -1) {
        mahaAshtamiIdx = i;
        if (!newData[dKey].festivals.includes('Maha Ashtami')) newData[dKey].festivals.push('Maha Ashtami');
      }
      if (p.tithi === 'Navami' && mahaNavamiIdx === -1) {
        mahaNavamiIdx = i;
        if (!newData[dKey].festivals.includes('Maha Navami')) newData[dKey].festivals.push('Maha Navami');
      }
      if (p.tithi === 'Dashami' && vijayaDashamiIdx === -1) {
        vijayaDashamiIdx = i;
        if (!newData[dKey].festivals.includes('Vijaya Dashami')) newData[dKey].festivals.push('Vijaya Dashami');
      }
      if (p.tithi === 'Ekadashi' && ekadashiIdx === -1) {
        ekadashiIdx = i;
        if (!newData[dKey].festivals.includes('Papankusha Ekadashi')) newData[dKey].festivals.push('Papankusha Ekadashi');
      }
      if (p.tithi === 'Purnima' && purnimaIdx === -1) {
        purnimaIdx = i;
        if (!newData[dKey].festivals.includes('Kojagrat Purnima')) newData[dKey].festivals.push('Kojagrat Purnima');
      }
    }

    // Continuous Dashain Public Holiday Block (from Fulpati to Dwadashi)
    if (fulpatiIdx !== -1) {
      const dwadashiIdx = (ekadashiIdx !== -1) ? ekadashiIdx + 1 : (vijayaDashamiIdx !== -1 ? vijayaDashamiIdx + 2 : fulpatiIdx + 5);
      for (let k = fulpatiIdx; k <= Math.min(dwadashiIdx, dailyPanchangs.length - 1); k++) {
        ...
        const hasDashainName = ['Fulpati', 'Maha Ashtami', 'Maha Navami', 'Vijaya Dashami', 'Papankusha Ekadashi'].some(f => newData[dKey].festivals.includes(f));
  ...
  let laxmiIdx = dailyPanchangs.findIndex(d => (d.month === 7 || d.month === 8) && d.paksha === 'Krishna' && (d.tithi === 'Amavasya' || d.sunsetPanchang.tithi === 'Amavasya'));
  if (laxmiIdx !== -1) {
    ...
    if (kukurIdx !== -1) {
      ...
      if (!newData[dKey].festivals.includes('Kukur Tihar')) newData[dKey].festivals.push('Kukur Tihar');
    }

    // Laxmi Puja
    const laxmiKey = dailyPanchangs[laxmiIdx].date;
    if (!newData[laxmiKey].festivals.includes('Laxmi Puja')) newData[laxmiKey].festivals.push('Laxmi Puja');
    ...
    let bhaiIdx = -1;
    if (govardhanIdx !== -1) {
      for (let k = govardhanIdx + 1; k <= Math.min(govardhanIdx + 2, dailyPanchangs.length - 1); k++) {
    ...
    const chhathIdx = dailyPanchangs.findIndex((d, idx) => idx > laxmiIdx && d.paksha === 'Shukla' && d.tithi === 'Shashthi');
    if (chhathIdx !== -1) {
      const dKey = dailyPanchangs[chhathIdx].date;
      if (!newData[dKey].festivals.includes('Chhath Puja')) newData[dKey].festivals.push('Chhath Puja');
    }

    // Haribodhini Ekadashi (Shukla Ekadashi after Laxmi Puja)
    const haribodhiniIdx = dailyPanchangs.findIndex((d, idx) => idx > laxmiIdx && d.paksha === 'Shukla' && d.tithi === 'Ekadashi');
    if (haribodhiniIdx !== -1) {
