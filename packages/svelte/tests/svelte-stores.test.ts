...
describe('@bsday.js/svelte Stores & Primitives', () => {
  describe('createNepaliCalendar', () => {
    it('initializes svelte store state with given year and month', () => {
      const cal = createNepaliCalendar({ initialYear: 2081, initialMonth: 5, locale: 'ne' });

      expect(get(cal.year)).toBe(2081);
      expect(get(cal.month)).toBe(5);
      expect(get(cal.currentMonthName)).toBe('भाद्र');
      expect(get(cal.matrix).length).toBe(6);
    });
    ...
    it('navigates next and previous months across year boundaries', () => {
      ...
      cal.goToNextMonth();
      expect(get(cal.year)).toBe(2081);
      expect(get(cal.month)).toBe(1);

      cal.goToPrevMonth();
      expect(get(cal.year)).toBe(2080);
      expect(get(cal.month)).toBe(12);
    });
    ...
    it('navigates years and sets specific month', () => {
      ...
      cal.setYear(2085);
      expect(get(cal.year)).toBe(2085);

      cal.setMonth(10);
      expect(get(cal.month)).toBe(10);
    });
    ...
    it('provides accessible ARIA grid props and cell props', () => {
      ...
      const firstCell = get(cal.matrix)[0][0];
  ...
  describe('createNepaliDatePicker', () => {
    it('manages open/close toggle state and formatted value', () => {
      ...
      expect(get(picker.isOpen)).toBe(false);
      expect(get(picker.formattedValue)).toBe('2081/05/15');

      picker.open();
      expect(get(picker.isOpen)).toBe(true);
      ...
      expect(get(picker.formattedValue)).toBe('2081/06/20');
  ...
  describe('createNepaliRangePicker', () => {
    it('handles two-step range selection and hover states', () => {
      ...
      expect(get(rangePicker.startDate)).toBe('2081/05/10');
      expect(get(rangePicker.endDate)).toBe('2081/05/15');
      ...
      expect(get(rangePicker.startDate)).toBe('2081/05/20');
      expect(get(rangePicker.endDate)).toBeNull();

      // Set hover
      rangePicker.setHoverDate('2081/05/25');
      expect(get(rangePicker.hoverDate)).toBe('2081/05/25');
      expect(rangePicker.isDateInRange('2081/05/22')).toBe(true);

      // Select end
      rangePicker.selectDate('2081/05/28');
      expect(get(rangePicker.startDate)).toBe('2081/05/20');
      expect(get(rangePicker.endDate)).toBe('2081/05/28');

      // Clear
      rangePicker.clear();
      expect(get(rangePicker.startDate)).toBeNull();
      expect(get(rangePicker.endDate)).toBeNull();
    });
  });

  describe('BS Prefix & Backward Compatibility Aliases', () => {
    it('ensures createBS* store factories match createNepali* aliases', () => {
